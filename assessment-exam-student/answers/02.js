...
const countWhich = function(list, cb) {
  ...
  /* IMPLEMENT ME */
  //checking if the first argument is an array, if it is, we can move on
  if (!Array.isArray(list)) {
    //code should return false if it's not an array
    return false;

  }

  //creating a counter to keep track
  let counter = 0;
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    //implmenting the callback function
  
    if (cb(list[i])) {
  
      //this should increment the count for the truth values found
      counter++;
