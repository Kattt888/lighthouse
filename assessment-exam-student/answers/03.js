...
const range = function(count, skipZero, descending) {
  ...
  //checking if the count is a number
  if (typeof count !== 'number') {
  //return empty array iif count is not a number
    return [];
  }
  //creating a range for the array
  const rangeArray = [];

  let rangeStart;
  
  if (skipZero) {
    //setting the start to 1 if skipzero is true
    rangeStart = 1;
  } else {
    //setting start to 0 if skipzero is false
    rangeStart = 0;
  }

  if (descending) {
    //for the descending order
    for (let i = count + rangeStart - 1; i >= rangeStart; i--) {
      rangeArray.push(i);
    }
  } else {
    //for ascending order
    for (let i = rangeStart; i < count + rangeStart; i++) {
      rangeArray.push(i);
    }
  }
  //return final array
